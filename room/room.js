...
const state = { session: null, tab: "chat", messages: [], stories: [], replyTo: null, timers: [], audio: null, audioUrl: null };
...
function renderChat(){
  ...
  body.innerHTML=`<div id="messageList" class="message-list"></div><form id="composer" class="composer"><p id="typing" class="typing hidden">the other heart is typing…</p><div id="replyPreview"></div><div class="composer-row"><textarea id="draft" placeholder="write something only they can read…" rows="1" maxlength="4000"></textarea><button class="send-button" aria-label="Send">➤</button></div><p id="errorBox" class="error-note hidden"></p></form>`;
  document.getElementById("composer").onsubmit=sendMessage; document.getElementById("draft").oninput=typingPing; paintMessages();
}
...
function paintReply(){const box=document.getElementById("replyPreview");if(!box)return;box.innerHTML=state.replyTo?`<div class="reply-preview"><span>Replying: ${escapeHtml(state.replyTo.text.slice(0,80))}</span><button type="button">×</button></div>`:"";if(box.querySelector("button"))box.querySelector("button").onclick=()=>{state.replyTo=null;paintReply()}}
async function loadMessages(){try{const result=await api("/api/room/messages",{},state.session.token);state.messages=await Promise.all(result.messages.map(async m=>({...m,text:await decryptText(state.session.roomKey,m.ciphertext,m.iv)})));paintMessages()}catch(error){if(error.message==="locked")lock()}}
